...
for df in reader:
    ...
    for i in range(len(df)):
        dis1 = df["position"].iloc[i] - 6
        dis2 = df["position"].iloc[i] + 6
        if df["strand"].iloc[i] == "-":
            for t in range(5):
                if i-t > 0 and df["position"].iloc[i-t] > dis1 and df["position"].iloc[i] > df["position"].iloc[i-t]:        
            ...
            for t in range(5):
                if i+t < len(df) and df["position"].iloc[i+t] < dis1 and df["position"].iloc[i] < df["position"].iloc[i+t]:        
